 ...
 * </code>
 *
 * $Horde: framework/VFS/VFS/musql.php,v 1.43 2006/03/12 18:31:46 jan Exp $
 *
...
class VFS_musql extends VFS_sql {
    ...
    function changePermissions($path, $name, $permission)
    {
        ...
        foreach ($fileList as $file) {
            if ($file[1] == $this->_params['user'] || $file[2] & VFS_FLAG_WRITE) {
                $result = $this->_db->query(sprintf('UPDATE %s SET vfs_perms = ?
                                                    WHERE vfs_id = ?',
                                                    $this->_params['table']),
                                            array($perm, $file[0]));
                return $result;
