 ...
 * </pre>
 *
 * $Horde: framework/VFS/VFS/sql.php,v 1.119 2006/03/12 18:31:46 jan Exp $
 *
...
class VFS_sql extends VFS {
    ...
    function size($path, $name)
    {
        ...
        return strlen($this->_readBlob($this->_params['table'], 'vfs_data',
    ...
    function deleteFolder($path, $name, $recursive = false)
    {
        ...
        $sql = sprintf('DELETE FROM %s WHERE vfs_path %s',
                       $this->_params['table'],
                       (empty($folderPath) && $this->_write_db->dbsyntax == 'oci8') ? ' IS NULL' : ' LIKE ' . $this->_write_db->quote($this->_getNativePath($folderPath, '%')));
        $this->log($sql, PEAR_LOG_DEBUG);
        ...
        $sql = sprintf('DELETE FROM %s WHERE vfs_path %s',
                       $this->_params['table'],
                       (empty($path) && $this->_write_db->dbsyntax == 'oci8') ? ' IS NULL' : ' = ' . $this->_write_db->quote($folderPath));
        $this->log($sql, PEAR_LOG_DEBUG);
        ...
        $sql = sprintf('DELETE FROM %s WHERE vfs_path %s AND vfs_name = ?',
                       $this->_params['table'],
                       (empty($path) && $this->_write_db->dbsyntax == 'oci8') ? ' IS NULL' : ' = ' . $this->_write_db->quote($path));
        $values = array($name);
        $this->log($sql, PEAR_LOG_DEBUG);
    ...
    function getFolderSize($path = null, $name = null)
    {
        ...
        $where = (is_null($path)) ? null : sprintf('WHERE vfs_path = %s', ((empty($path)) ? '""' : $this->_db->quote($this->_convertPath($path))));
