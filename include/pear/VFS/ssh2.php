 ...
 *                       than 22.</pre>
 *
 * $Horde: framework/VFS/VFS/ssh2.php,v 1.9 2006/03/30 08:03:09 selsky Exp $
 *
 * Copyright 2006 Cliff Green <green@umdnj.edu>
 *
...
class VFS_ssh2 extends VFS {
    ...
    function read($path, $name)
    {
        ...
        if (!@ssh2_scp_recv($this->_stream, $this->_getPath($path, $name), $tmpFile)) {
    ...
    function changePermissions($path, $name, $permission)
    {
        ...
        if (!@ssh2_exec($this->_stream, 'chmod ' . $permission . ' ' . $this->_getPath($path, $name))) {
    ...
    function copy($path, $name, $dest, $autocreate = false)
    {
        ...
        if ($this->isFolder($path, $name)) {
            ...
            $tmpFile = $this->_getTempFile();
            $fetch = @ssh2_scp_recv($this->_stream, $orig, $tmpFile);
            if (!$fetch) {
                unlink($tmpFile);
                return PEAR::raiseError(sprintf(_("Failed to copy from \"%s\"."), $orig));
            }

            if (!@ssh2_scp_send($this->_stream, $tmpFile, $this->_getPath($dest, $name))) {
                unlink($tmpFile);
    ...
    {
        if ($stream = @ssh2_exec($this->_stream, 'cd ' . $path . '; pwd')) {
            stream_set_blocking($stream, true);
            $this->_cwd = trim(fgets($stream));
            fclose($stream);
            return true;
    ...
    /**
     * Disconnects from the SSH2 server and cleans up the connection.
     *
     * @access private
     */
