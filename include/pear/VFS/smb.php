 ...
 *                          module.
 *
 * $Horde: framework/VFS/VFS/smb.php,v 1.17 2006/03/30 08:03:09 selsky Exp $
 *
...
class VFS_smb extends VFS {
    ...
    function read($path, $name)
    {
        ...
        $temp = $this->_getTempFile();
        $cmd = array('get \"' . $name . '\" ' . $temp);
     ...
     * @param boolean $autoc       reate  Automatically create directories?
     *
     * @return boolean|PEAR_Error  True on success or a PEAR_Error on failure.
     */
     ...
     */
    function _createTempFile($path, $name)
    {
        list($path, $name) = $this->_escapeShellCommand($path, $name);
        $temp = $this->_getTempFile();
        $cmd = array('get \"' . $name . '\" ' . $temp);
