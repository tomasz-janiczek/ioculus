 ...
 *
 * $Horde: framework/VFS/VFS.php,v 1.96 2006/03/30 08:03:09 selsky Exp $
 *
...
class VFS {
    ...
    function getFolderSize($path = null, $name = null)
    {
        ...
        foreach ($object_list as $key => $val) {
            if (isset($val['subdirs'])) {
                ...
                $size += @$this->getFileSize($root, $key);
            }
        }
    ...
    function getQuota()
    {
        if (empty($this->_params['vfs_quotalimit'])) {
            ...
            if (is_a($usage, 'PEAR_Error')) {
                ...
                return array('usage' => $this->getVFSSize(), 'limit' => $this->_params['vfs_quotalimit']);
            }
        }
    }
    ...
    function _checkQuotaWrite($mode, &$data)
    {
        if ($this->_params['vfs_quotalimit'] != -1) {
            ...
            if (($this->getVFSSize() + $filesize) > $this->_params['vfs_quotalimit']) {
    ...
    function _checkQuotaDelete($path, $name)
    {
        if (($this->_params['vfs_quotalimit'] != -1) &&
            !empty($this->_vfsSize)) {
            $filesize = $this->getFileSize($path, $name);
            if (is_a($filesize, 'PEAR_Error')) {
                return PEAR::raiseError(_("Unable to read VFS file (getFileSize() failed)."));
            }
            $this->_vfsSize -= $filesize;
        }
