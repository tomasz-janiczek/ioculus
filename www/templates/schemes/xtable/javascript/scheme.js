	...
	*/	
	...
	var arrowImagePath = "/www/templates/schemes/xtable/images/";	// Path to arrow images
	...
	function initTableWidget(objId,width,height,sortArray)
	{
		...
		height = height + '';
		var obj = $(objId);
		obj.parentNode.className='scheme_xtable';
...
schemeXTable = new iScheme();
Object.extend(schemeXTable, {
	name : 'xtable',
	events : {
		'.scheme_xtable a:mouseover' : function(element)
		{
			Element.addClassName(element, 'tableWidget_a_over');
		},

		'.scheme_xtable a:mouseout' : function(element)
		{
			Element.removeClassName(element, 'tableWidget_a_over');
		},

		'.scheme_xtable table thead tr td input.checkbox:click' : function(element)
		{
			var index = element.parentNode.cellIndex;
			var table = element.parentNode.parentNode.parentNode.parentNode;
			var rows = $A(table.tBodies[0].rows);
			
			rows.each(function(row) {
				var input = row.cells[index].getElementsByTagName('input')[0];
				if (input && input.type.toLowerCase() == 'checkbox') input.checked = !input.checked;
			});
		}
	},
	...
	{	
		var schemes = this.getAllInstances();
		if (!schemes) return;

		schemes.each(function(scheme) {
				var areas = $A(scheme.getElementsByTagName('table'));
				if (areas) areas.each(function(area) {
					var size = Element.getDimensions(area);
					initTableWidget(area, '100%', 'auto', Array(false,'S', 'S', 'S','S', 'S', 'S'));
				});
		});
	},
	
	getSelected : function(id, serialize)
	{
		if (!serialize) serialize = true;
		
		var table = $(id).getElementsByTagName('table')[0];
		if (!table) return null;
		
		var boxes = $A(table.tBodies[0].getElementsByTagName('input'));
		var list = Array();
		
		boxes.each(function(box) {
			if (box && box.type.toLowerCase() == 'checkbox' && box.checked) list.push(box.value);
		});
		
		if (!serialize) return list;
		else return list.implode(',');
	}
});

Core.registerScheme(schemeXTable);
